...
void process(){
 ...
 // std::cout <<"uav_cloud_count: "<< uav_cloud->points.size () << std::endl;
  ...
  for (size_t i = 0; i <size; ++i)
  {
    ...
    if( (cloud_target->points[i].x*cloud_source->points[i].x)>0 && 
      cloud_target->points[i].x< (cloud_source->points[i].x+threshold) && cloud_target->points[i].x > (cloud_source->points[i].x-threshold))
    {
      if( (cloud_target->points[i].y*cloud_source->points[i].y)>0 && 
      cloud_target->points[i].y< (cloud_source->points[i].y+threshold) && cloud_target->points[i].y > (cloud_source->points[i].y-threshold))
      { 
        if( (cloud_target->points[i].z*cloud_source->points[i].z)>0 && 
        cloud_target->points[i].z< (cloud_source->points[i].z+threshold) && cloud_target->points[i].z > (cloud_source->points[i].z-threshold))
        {
          match_score++;
        }
      }
...
int main (int argc, char** argv)
{
  ...
  pcSubscriber1 = node_->subscribe("/octomap_point_cloud_centers",   200, pcCallback_ugv);
